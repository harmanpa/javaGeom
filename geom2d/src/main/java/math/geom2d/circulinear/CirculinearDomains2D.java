...
public class CirculinearDomains2D {
    ...
    public final static CirculinearDomain2D computeBuffer(
        ...
        for (CirculinearContour2D contour : domain.contours()) {
            ...
            for (CirculinearContinuousCurve2D simpleCurve
                ...
                CirculinearContour2D boundary
                        = new BoundaryPolyCirculinearCurve2D<>(
        ...
        return new GenericCirculinearDomain2D(
                new CirculinearContourArray2D<>(rings));
    }

    /**
     * Computes the rings that form the domain of a circulinear curve which does
     * not self-intersect.
     *
     * @param curve
     * @param d
    ...
    public final static Collection<CirculinearContour2D>
        ...
        CirculinearCurves2D.splitContinuousCurve(parallel1).stream()
                .filter((split) -> (CirculinearCurves2D.findIntersections(curve, split).isEmpty()))
        ...
        for (CirculinearContinuousCurve2D split : curves) {
            rings.add(new BoundaryPolyCirculinearCurve2D<>(
                            split.smoothPieces(), split.isClosed()));
        }

        // prepare an array to store the set of rings
        ArrayList<CirculinearContour2D> rings2 = new ArrayList<>();

        // iterate on the set of rings
        for (CirculinearContour2D ring : rings) // split rings into curves which do not self-intersect
        {
            for (CirculinearContinuousCurve2D split
                ...
                rings2.add(new BoundaryPolyCirculinearCurve2D<>(
                                split.smoothPieces(), split.isClosed()));
            }
        }
