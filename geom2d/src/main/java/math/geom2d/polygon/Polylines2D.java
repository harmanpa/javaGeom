...
public abstract class Polylines2D {
    ...
    public static List<Point2D> simplifyPolyline(List<Point2D> vertices,
        ...
        List<Point2D> newVerts = new ArrayList<>(inds.length + 2);
    ...
    static List<Point2D> simplifyClosedPolyline(List<Point2D> vertices,
            double distMax) {
     ...
     */
    private static int[] recurseSimplify(List<Point2D> vertices, int first,
    ...
    /**
     * Checks if the input polyline has multiple vertices.Extremities are tested
     * if the polyline is closed (second argument is true).
     *
     * @param polyline
     * @param closed
     * @return
    ...
    /**
     * Return all intersection points between the 2 polylines. This method
     * implements a naive algorithm, that tests all possible edge couples. It is
     * supposed that only one point is returned by intersection.
     *
