...
/**
 * Computation of the convex hull using Graham scan algorithm. Note that in its
 * current implementation, this algorithms fails when some points are colinear.
 *
 * @author dlegland
 *
 */
public class GrahamScan2D implements ConvexHull2D {
    ...
    public Polygon2D convexHull(Collection<? extends Point2D> points) {
        ...
        // create a sorted set
        List<Point2D> sorted = new ArrayList<>(nbPoints);
        ...
        for (int i = 3; i < nbPoints; i++) {
            while (m > 0 && Point2D.ccw(sorted.get(m), sorted.get(m - 1),
                    sorted.get(i)) >= 0) {
                m--;
            }
            m++;
