...
//            return new Circle2D(xc, yc, chord / 2);
    ...
    /**
     * Define center by point, major and minor semi axis lengths, orientation
     * angle, and boolean flag for direct ellipse.
     */
...
//			return new Circle2D(0, 0, r1);
     ...
     * PolarVector2D is positive if point lies Refs :
     * <p>
...
//            		+" +/- "+Math.toDegrees(dPhi)+", k = "+k);
    ...
    public Ellipse2D parallel(double d) {
        return new Ellipse2D(xc, yc, Math.max(0, r1 + d), Math.max(0, r2 + d), theta, direct);
    }
    ...
    /**
     * Returns true if this ellipse is similar to a circle, i.e. has same length
     * for both r1 and r2.
     */
    ...
    /**
     * Returns an approximation of the signed distance to the ellipse. In the
     * current implementation, the ellipse is converted to a polyline.
     */
    ...
    /**
     * Transforms the coordinates of the given point to the coordinate system if
     * the ellipse were transformed to a unit circle.
     *
    ...
    /**
     * Clip the ellipse by a box. The result is an instance of CurveSet2D, which
     * contains only instances of Ellipse2D or EllipseArc2D. If the ellipse is
     * not clipped, the result is an instance of CurveSet2D which contains 0
     * curves.
     */
    ...
    public Collection<Point2D> intersections(LinearShape2D line) {
        ...
        AffineTransform2D toUnit;
        try {
            toUnit = sca.chain(rot).chain(tra).invert();
    ...
    public int hashCode() {
        int hash = 3;
        hash = 73 * hash + (int) (Double.doubleToLongBits(this.xc) ^ (Double.doubleToLongBits(this.xc) >>> 32));
        hash = 73 * hash + (int) (Double.doubleToLongBits(this.yc) ^ (Double.doubleToLongBits(this.yc) >>> 32));
        hash = 73 * hash + (int) (Double.doubleToLongBits(this.r1) ^ (Double.doubleToLongBits(this.r1) >>> 32));
        hash = 73 * hash + (int) (Double.doubleToLongBits(this.r2) ^ (Double.doubleToLongBits(this.r2) >>> 32));
        hash = 73 * hash + (int) (Double.doubleToLongBits(this.theta) ^ (Double.doubleToLongBits(this.theta) >>> 32));
        hash = 73 * hash + (this.direct ? 1 : 0);
        return hash;
    }
