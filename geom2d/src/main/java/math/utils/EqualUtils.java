...
public final class EqualUtils {
     ...
     * equality is defined by Tolerance2D.get(). @param aThis @param aThat
     * @return True if aThis is less than or approximately equal to aTh
     *
     * a
     * t
     * .
    ...
    static public boolean isLE(double aThis, double aThat) {
        ...
        if (Math.abs(aThat - aThis) <= Tolerance2D.get()) {
            return true;
        }
    ...
    /**
     * Implements >= for doubles based on approximate equality. Approximate
     * equality is defined by Tolerance2D.get().
     *
     * @param aThis
     * @param aThat
