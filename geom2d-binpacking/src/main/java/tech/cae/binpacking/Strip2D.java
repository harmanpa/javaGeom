...
public class Strip2D implements Comparable<Strip2D> {
    ...
    public Strip2D(int id, double[][] original_poly, double offset, Double segment_max_length) throws PackingException {
        this.id = id;
        if (offset <= 0) {
            throw new PackingException("Offset must be > 0");
        }
    ...
    @Override
    public int compareTo(Strip2D pl) {
